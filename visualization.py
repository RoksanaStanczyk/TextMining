...
def describe_data(df):
    category = df.groupby("airline_sentiment")
    category.size().sort_values(ascending=False).plot.bar()
    plt.xlabel("Airline sentiment")
    plt.ylabel("Number of airline sentiment")
    plt.show()


def token_visualization(df: pd.DataFrame):
    df = df['text']
    ...
    plt.title(f'10 najczęściej występujących tokenów')
    plt.ylabel('Token')
    plt.xlabel('Ilość')
    ...
    plt.title(f'10 najważniejszych tokenów')
    plt.ylabel('Token')
    plt.xlabel('Waga')
    plt.show()
